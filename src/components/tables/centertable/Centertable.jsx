...
import { deleteCenter, getCenter } from '../../../api/center_api';
...
export default function EnhancedTable() {
    ...
    const { setCenters, loadCenter, setLoadCenter } = useContext(DataContext);
    const navigate = useNavigate();

    useEffect(() => {
        const fetchData = async () => {
            try {
                const mappedData = await getCenter();
                setCenters(mappedData.data);
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.idcentro);
    ...
    const handleClick = (event, idcentro) => {
        const selectedIndex = selected.indexOf(idcentro);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, idcentro);
    ...
    const handleDelete = async () => {
        if (window.confirm("¿Estás seguro de que deseas eliminar los elementos seleccionados?")) {
            try {
                await Promise.all(selected.map(item => deleteCenter(item)));
                const updatedRows = rows.filter(row => !selected.includes(row.idcentro));
                setRows(updatedRows);
    ...
    const handleEdit = (row) => {
        ...
        navigate(`/centro/modificar/`, { state: { row } });
    };
    ...
    const filteredRows = rows.filter((row) =>
        ...
        row.organismo.toLowerCase().includes(searchTerm.toLowerCase())
    );
                    ...
                    >
                        ...
                        <TableBody>
                            {visibleRows.map((row, index) => {
                                const isItemSelected = selected.indexOf(row.idcentro) !== -1;
                                const labelId = `enhanced-table-checkbox-${index}`;

                                return (
                                    <TableRow
                                        hover
                                        onClick={(event) => handleClick(event, row.idcentro)}
                                        role="checkbox"
