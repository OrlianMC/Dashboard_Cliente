...
const headCells = [
  { id: 'idareadeconocimiento', numeric: false, disablePadding: false, label: 'ID' },
...
export default function EnhancedTable() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.idareadeconocimiento);
  ...
  const handleClick = (event, idareadeconocimiento) => {
    const selectedIndex = selected.indexOf(idareadeconocimiento);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, idareadeconocimiento);
  ...
  const handleEdit = (row) => {
    ...
    navigate(`/areadeconocimiento/modificar/`, { state: { row } });
  };
          ...
          >
            ...
            <TableBody>
              {visibleRows.map((row, index) => {
                const isItemSelected = selected.indexOf(row.idareadeconocimiento) !== -1;
                const labelId = `enhanced-table-checkbox-${index}`;

                return (
                  <TableRow
                    hover
                    onClick={(event) => handleClick(event, row.idareadeconocimiento)}
                    role="checkbox"
                    aria-checked={isItemSelected}
                    tabIndex={-1}
                    key={row.idareadeconocimiento}
