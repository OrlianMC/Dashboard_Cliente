...
import { deleteCountry, getCountry } from '../../../api/country_api';
...
const headCells = [
    { id: 'idpais', numeric: false, disablePadding: false, label: 'ID' },
...
export default function EnhancedTable() {
    ...
    const { setCountries, loadCountry, setLoadCountry } = useContext(DataContext);
    const navigate = useNavigate();

    useEffect(() => {
        const fetchData = async () => {
            try {
                const mappedData = await getCountry();
                setCountries(mappedData.data);
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.idpais);
    ...
    const handleClick = (event, idpais) => {
        const selectedIndex = selected.indexOf(idpais);
        let newSelected = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, idpais);
    ...
    const handleDelete = async () => {
        if (window.confirm("¿Estás seguro de que deseas eliminar los elementos seleccionados?")) {
            try {
                await Promise.all(selected.map(item => deleteCountry(item)));
                const updatedRows = rows.filter(row => !selected.includes(row.idpais));
                setRows(updatedRows);
    ...
    const handleEdit = (row) => {
        ...
        navigate(`/pais/modificar/`, { state: { row } });
    };
                    ...
                    >
                        ...
                        <TableBody>
                            {visibleRows.map((row, index) => {
                                const isItemSelected = selected.indexOf(row.idpais) !== -1;
                                const labelId = `enhanced-table-checkbox-${index}`;

                                return (
                                    <TableRow
                                        hover
                                        onClick={(event) => handleClick(event, row.idpais)}
                                        role="checkbox"
